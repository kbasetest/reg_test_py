SERVICE = reg_test_py
SERVICE_CAPS = reg_test_py
SPEC_FILE = reg_test_py.spec
URL = https://kbase.us/services/reg_test_py
DIR = $(shell pwd)
LIB_DIR = lib
SCRIPTS_DIR = scripts
TEST_DIR = test
...
build-test-script:
	...
	# TODO NOW set coverage report locations
	echo 'pytest  -vv --cov=$(LIB_DIR)/$(SERVICE_CAPS) --cov-report=html --cov-report=xml $(TEST_SPEC)' >> $(TEST_DIR)/$(TEST_SCRIPT_NAME)
